...
ana_dir = '/Users/xinian.zuo/Downloads/2dReHo-MCI/2dReHo-stats';
...
%matplot colormap
fcolor = [ccs_dir '/vistool/colormaps_reference_03.hires.png'];
cmap_pus = ccs_extractcolormaps(fcolor);
cmap_tdmean = cmap_pus{3}; cmap_tdmean(127:129,:) = 0.5;
%Left Hemisphere
...
%% Figure 1: two-sample t-tests
cmin = 2; FWEthresh = 0.025; 
...
%% Figure 1: average
fmask = [ana_dir '/g4v2.reho.lh/mask.mgh'];%lh
tmpmaskvol = load_mgh(fmask);
freho = [ana_dir '/lh.reho.sm6.nii.gz'];
...
fmreho = [ana_dir '/lh.mreho.nc.nii.gz'];
err1 = save_nifti(samplehdr, fmreho);
%mci
samplehdr.vol = mean(rehovol(:,40:end),2);
fmreho = [ana_dir '/lh.mreho.amci.nii.gz'];
err2 = save_nifti(samplehdr, fmreho);

fmask = [ana_dir '/g4v2.reho.rh/mask.mgh'];%rh
tmpmaskvol = load_mgh(fmask);
freho = [ana_dir '/rh.reho.sm6.nii.gz'];
tmphdr = load_nifti(freho); rehovol = squeeze(tmphdr.vol);
fsample = ['/Users/xinian.zuo/Downloads/dnb_healthybrain/' ...
...
%nc
samplehdr.vol = mean(rehovol(:,1:39),2);
fmreho = [ana_dir '/rh.mreho.nc.nii.gz'];
err3 = save_nifti(samplehdr, fmreho);
%mci
samplehdr.vol = mean(rehovol(:,40:end),2);
fmreho = [ana_dir '/rh.mreho.amci.nii.gz'];
err4 = save_nifti(samplehdr, fmreho);

%% Figure 1: rendering mean maps
fig_dir = [ana_dir '/figures']; maps = {'amci', 'nc'};
for mapid=1:numel(maps);
    maps{mapid}
    %lh
    fMAP = [ana_dir '/fsLR/lh.mreho.' maps{mapid} '.32k.gii'];
    lh_map = gifti(fMAP); corrMap_lh = double(lh_map.cdata);
    figure('Units', 'pixel', 'Position', [100 100 800 800]); axis off
    SurfStatView(corrMap_lh, surfConte69_inflated_lh, ' ', 'white', 'true'); 
    colormap(cmap_icc); SurfStatColLim([0 1]);
    set(gcf, 'PaperPositionMode', 'auto');
    figout = [ana_dir '/figures/' maps{mapid} '.mreho.lh.png'];
    print('-dpng', '-r300', figout); close
    %rh
    fMAP = [ana_dir '/fsLR/rh.mreho.' maps{mapid} '.32k.gii'];
    rh_map = gifti(fMAP); corrMap_rh = double(rh_map.cdata);
    figure('Units', 'pixel', 'Position', [100 100 800 800]); axis off
    SurfStatView(corrMap_rh, surfConte69_inflated_rh, ' ', 'white', 'true'); 
    colormap(cmap_icc); SurfStatColLim([0 1]);
    set(gcf, 'PaperPositionMode', 'auto');
    figout = [ana_dir '/figures/' maps{mapid} '.mreho.rh.png'];
    print('-dpng', '-r300', figout); close
end